...
export default function App() {
  ...
  return (
    <SafeAreaView style={ flex:1, padding:16 }>
      <ScrollView>
        <Text style={ fontSize:22, fontWeight:'600', marginBottom:12 }>Offline-first Field Ops – Mobile</Text>
        <Button title="Ping API" onPress={async()=>{
          try {
            const r = await fetch('http://10.0.2.2:4102/'); // Android emulator loopback
